...
这份文档记录 SmartGuard AI 前端后续可以继续优化的方向。相关内容不作为当前版本的功能承诺，而是作为体验完善、验收准备和线上稳定性提升的参考。
...
已有页面使用 SWR。实时性要求高的页面可以设置刷新间隔，普通页面则保持按需刷新，避免不必要的请求开销。
...
列表和图表加载时，优先使用骨架屏或局部 loading，减少整页空白带来的等待感。
...
前端调试日志应避免输出密码、完整 token、身份证号或其他敏感字段。
...
页面负责组织数据和布局，复杂交互可以下沉为组件。单次使用的逻辑保持简单，重复出现的卡片、空状态、弹窗可以统一。
...
图表数据应来自后端真实 API；如使用演示数据，应在页面或材料中明确标注。
