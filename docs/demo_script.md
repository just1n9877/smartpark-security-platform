...
这段演示用于说明系统不是“只会报一次警”，而是能根据用户反馈调整后续策略。验收或演示时，可以用同一段视频做前后对比。
...
如使用 `RepCount`、`LLSP` 等公开视频，可在演示材料中说明其用途为算法验证视频。
...
演示过程中可以打开后端日志窗口，观察 `debounce M=` 和自动调参日志。
...
对同一批告警提交 `false_positive`。提交多条反馈后，更容易观察滚动窗口统计和阈值变化。
...
当前闭环属于规则阈值层面的反馈优化，不等同于完整生产级在线学习系统，也不承诺完全消除误报。
